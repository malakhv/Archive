...
/**
 * Class contains some methow for working with {@link InputStream}.
 *
...
class StreamUtils {
     ...
     * */
    public static String toStringWithThrow(InputStream is, boolean doClose) throws IOException {
        ...
        try {
            String str;
            while ((str = bReader.readLine()) != null) sBuilder.append(str).append("\n");
        } finally {
            bReader.close();
            if (doClose) is.close();
        }
    ...
    public static String toString(InputStream is, boolean doClose) {
        try {
            return toStringWithThrow(is, doClose);
        } catch (IOException e) {
            return null;
        }
    }
