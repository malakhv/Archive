...
import static appinst.updater.nemo.appinst.ShellUtils.*;
...
public class PackageUtils {
     ...
     * пользователь должен обладать root правами, или приложение должно быть системным. Требует
     * наличия разрешения {@code android.permission.INSTALL_PACKAGES}.
     *
     * @param context текущий контекст приложения
     * @param file объект, представляющий файл приложения на диске
     * @param params параметры командной строки менеджера пакетов
     * */
    public static int installSilent(Context context, File file, String params) {
        return installSilent(context, file == null ? null : file.getAbsolutePath(), params);
    }
     ...
     * @param params параметры командной строки менеджера пакетов
     * */
    public static int installSilent(Context context, String fileName, String params) {
        ...
        /* Выполнение команды */
        CmdResult cmdResult = ShellUtils.execCmd(cmd, !isSystemApp(context), true);
    ...
    /**
     * Удаляет прилождение в нормальном режиме с использованием интента.
     *
     * @param context текущий контекст приложения
     * @param packageName имя пакета приложения, которое необходимо удалить
     * */
