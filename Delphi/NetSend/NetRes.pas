...
begin
  inherited Create;
  FResScope := RESOURCE_CONTEXT;//RESOURCE_GLOBALNET;
  FResType  := RESOURCETYPE_ANY;
  FResUsage := RESOURCEUSAGE_ATTACHED;//RESOURCEUSAGE_CONTAINER;
  if GetNetRes then
    CreateResList;
end;
...
begin
  ...
  repeat
    ...
    begin
      ...
      begin
        ...
        AddCompInfo(NRes);
        if FResScope = RESOURCE_GLOBALNET then
        begin
          if (PNetResLocal^[i].dwUsage and RESOURCEUSAGE_CONTAINER ) =
  ...
  FreeMem(PNetResLocal,Length(PNetResLocal^)*SizeOf(TNetResource));
